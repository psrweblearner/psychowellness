...
$edit_id = @$_GET['edit'];
$query = "select * from cancel where id='$edit_id'";
$run = mysqli_query($con,$query);
while($row= mysqli_fetch_array($run)){
			$edit_id1 =$row['id'];
	        $text_field= $row['text_field'];
	?>
<form method="post" action="edit_cancel.php?edit_form=<?php echo $edit_id1; ?>">
...
if(isset($_POST['update'])){
	...
	$update_query ="update cancel set text_field='$text_field' where id='$update_id'";
	if(mysqli_query($con,$update_query)){
		echo"<script language='javascript'>window.open('index.php?published=    Cancellation & Refund Policy has been published','_self')</script>";
	}
}
