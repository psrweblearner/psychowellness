...
$edit_id = @$_GET['edit'];
$query = "select * from intership where id='$edit_id'";
...
<form method="post" action="edit_intership.php?edit_form=<?php echo $edit_id1; ?>">
...
if(isset($_POST['update'])){
	...
	$text_field= $_POST['text_field'];
		...
		$update_query ="update intership set text_field='$text_field' where id='$update_id'";
	
	if(mysqli_query($con,$update_query)){
		
		
		
		echo"<script language='javascript'>window.open('index.php?published=Intership has been published','_self')</script>";
	}
}
